...
class PlayState extends MusicBeatState
{
	...
	public var spikeStand:Character;
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / (1000 * startSpeed), function(tmr:FlxTimer)
		{
			...
			introAssets.set('default', [
				'ui/ratings/' + ratingstype + '/ready',
				'ui/ratings/' + ratingstype + '/set',
				'ui/ratings/' + ratingstype + '/go'
			]);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			{
				if (songNotes[1] > -1) // get rid of invalid notes
				{
	...
	function generateStaticArrows(player:Int, animated:Bool = true, is3d:Bool = false):Void
	{
		for (i in 0...4)
		{
			...
			if (Note.CharactersWith3D.contains(opponent.curCharacter)
				&& player == 0
			...
			else if (((Note.CharactersWithPixel.contains(opponent.curCharacter) && player == 0)
				|| (Note.CharactersWithPixel.contains(boyfriend.curCharacter) && player == 1)))
			{
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'applecore')
		{
			if (poiping)
			{
				...
				if (ThisIntegerThatIDontKnowWhatDoes >= 2)
					poiping = false;
			}
			else if (canPoip)
			{
		...
		if (funnyFloatyBoys.contains(opponent.curCharacter.toLowerCase())
			&& canFloat
			&& orbit
			|| opponent.floater != 'false'
			&& canFloat
			&& orbit)
		{
	...
	override function beatHit()
	{
		...
		if (Assets.exists(Paths.chart('${curSong.toLowerCase()}/events'))) // json events that can be changed or removed
		{
				...
				if (curStep == event.step)
				{
					switch (event.name)
					{
						...
						case 'Play Animation':
							...
							{
								case 'opponent':
									opponent.playAnim(event.text, event.force);
									opponentmirror.playAnim(event.text, event.force);
								case 'girlfriend':
									gf.playAnim(event.text, event.force);
								case 'boyfriend':
									boyfriend.playAnim(event.text, event.force);
								case 'opponent2':
									opponent2.playAnim(event.text, event.force);
							}
						case 'Play Sound':
